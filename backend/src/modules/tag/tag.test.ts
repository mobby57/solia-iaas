...
describe('Tag Service', () => {
  ...
  it('should update a tag', async () => {
    const tag = await createTag(
      { label: 'Initial Tag', color: '#00ff00', targetId: 'target2', targetType: 'type2' },
      'default-tenant',
    );
  ...
  it('should delete a tag', async () => {
    const tag = await createTag(
      { label: 'Tag to Delete', color: '#123456', targetId: 'target3', targetType: 'type3' },
      'default-tenant',
    );
  ...
  it('should reject creating a tag without tenantId', async () => {
    await expect(
      createTag(
        { label: 'No Tenant Tag', color: '#000000', targetId: 'target4', targetType: 'type4' },
        '',
      ),
    ).rejects.toThrow();
  });
